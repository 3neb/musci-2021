module.exports = {
  ...
  async Player(message, Discord, client, Ytdl, options = {}, db) {
    ...
    setTimeout(async () => {
      ...
      .setAuthor(`Now running a song`)
      .setThumbnail(options.Play.Thumbnail)
      .setDescription(`[${options.Play.Title}](${options.Play.Link})`)
      .setColor(message.guild.member(message.client.user).roles.highest.hexColor)
      .setTimestamp()
      .setFooter(`${message.author.username} `, message.author.avatarURL());
 
        await Db.TextChannel.send(PlayEmbed).catch(() =>
          Db.TextChannnel.send(`Now running a song ${options.Play.Title}`)
        );
        Db.ExtraTime = 0;
      };
