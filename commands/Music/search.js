...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) return message.channel.send(new Discord.MessageEmbed().setColor(message.guild.member(client.user).roles.highest.hexColor)
.setDescription("Please Give Query To Search"))
        ...
        .then(async Msg => {
          ...
          if (Queue) {
            ...
            return message.channel
              .send(Embed)
              .catch(() =>
        message.channel.send(new Discord.MessageEmbed().setColor(message.guild.member(client.user).roles.highest.hexColor)
.setDescription("Song Has Been Added To Queue"))
              );
          }
