...
.setDescription("Please Join A Voice Channel To Play Music!"))
    ...
    if (ServerQueue) {
      ...
      .setTitle(`Now running a song`)
      .setColor(message.guild.member(client.user).roles.highest.hexColor)
      .setThumbnail(Playlist.bestThumbnail.url)
          .setDescription( 
            `[${Playlist.More.title}](${Playlist.More.url}) (${Playlist.More.estimatedItemCount === 100 ? "100 (Limit)" : Playlist.More.estimatedItemCount})`
          )
          .setTimestamp()
      .setFooter(`${message.author.username} `, message.author.avatarURL());
        ...
        const Embed = new Discord.MessageEmbed()
          ...
          .setDescription(`[${Song.Title}](${Song.Link})` )
        .setTimestamp()
        .setFooter(`${message.author.username} `, message.author.avatarURL());
